...
extern "C" void quasirandomGeneratorGPU(float *d_Output, unsigned int seed, unsigned int N)
{
    ...
    //quasirandomGeneratorKernel<<<256, threads>>>(d_Output, seed, N);

    //dim3 threads(256, QRNG_DIMENSIONS);
...
{
	//inverseCNDKernel<<<16, 1024>>>(d_Output, d_Input, N);
	//inverseCNDKernel<<<32, 512>>>(d_Output, d_Input, N);
	//inverseCNDKernel<<<64, 256>>>(d_Output, d_Input, N);
	inverseCNDKernel<<<128, 128>>>(d_Output, d_Input, N);
    //inverseCNDKernel<<<256, 64>>>(d_Output, d_Input, N);
    //inverseCNDKernel<<<512, 32>>>(d_Output, d_Input, N);
